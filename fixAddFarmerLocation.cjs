...
const newAutoDetect = `  const autoDetectLocation = () => {
    if (!navigator.geolocation) {
      alert("तुमच्या ब्राउझरमध्ये लोकेशन फीचर उपलब्ध नाही.");
      return;
    }
...
const newError = `        (error) => {
    ...
    requestLocation(true, 1); // Try high accuracy first, fallback to low accuracy
  };`;

code = code.replace(targetError, newError);

fs.writeFileSync('src/components/AddFarmerForm.tsx', code);
